...
const UUID = process.env.UUID || '78dc07f4-c73b-4935-a082-c904513d909e'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
const NEZHA_SERVER = process.env.NEZHA_SERVER || '';        // 哪吒v1填写形式: nz.abc.com:8008  哪吒v0填写形式：nz.abc.com
const NEZHA_PORT = process.env.NEZHA_PORT || '';            // 使用哪吒v1请留空，哪吒v0需填写
const NEZHA_KEY = process.env.NEZHA_KEY || '';              // 哪吒v1的NZ_CLIENT_SECRET或哪吒v0的agent密钥
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'tunnel.bguo.netlib.re';          // 固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiYmQ1MWYwMjgzMDk4NTNlODU1N2Q0MDM2NGM1NGVhZDEiLCJ0IjoiOTc4NmFkOWYtNGU1Yy00YzlkLThmOGItZThiYWFlNGZjNDU3IiwicyI6Ik1HTTNNRGM1WkRrdE5tRmpOeTAwT1dFeExUazVNRGN0WW1GbVpEZGtaVGt5TkRnNSJ9';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://json.zone.id
const ARGO_PORT = process.env.ARGO_PORT || 8001;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
const CFIP = process.env.CFIP || 'cdns.doon.eu.org';        // 节点优选域名或优选ip  
const CFPORT = process.env.CFPORT || 443;                   // 节点优选域名或优选ip对应的端口
const NAME = process.env.NAME || 'gb';                        // 节点名称
...
    }
    ...
    // 由于现在 sub.txt 存储的是 Base64 编码的 Clash YAML，
    // 删除单个节点的逻辑变得复杂且不可靠，保留原逻辑但知道它可能不再适用。
...
    const nodeName = NAME ? `${NAME}-${ISP}` : ISP;
    ...
    // --- 【修改开始】 转换 V2Ray 节点为 Clash YAML 结构 ---

    // 1. 定义 Clash 节点（Proxies）
    // 注意：VLESS flow: xtls-rprx-vision 需要 Clash Premium 内核支持
    const vlessProxy = {
        name: `VLESS-${nodeName}`,
        type: 'vless',
        server: CFIP,
...
    }
  } else if (UPLOAD_URL) {
    // 上传单个节点列表（可能已失效，因为现在生成的是 Clash YAML）
